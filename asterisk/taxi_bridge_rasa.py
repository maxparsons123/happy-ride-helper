#!/usr/bin/env python3
"""Taxi AI Asterisk Bridge - RASA MODE (16kHz STT)

This is a variant of taxi_bridge_v6.py with Rasa-style audio processing
enabled by default (8kHz→16kHz instead of 8kHz→24kHz).

For A/B testing STT accuracy between 16kHz and 24kHz modes.

Dependencies:
    pip install websockets numpy scipy

Usage:
    python3 taxi_bridge_rasa.py
...
# ═══════════════════════════════════════════════════════════════════════════
# RASA MODE ENABLED BY DEFAULT
# This variant uses 8kHz→16kHz resampling instead of 8kHz→24kHz
# Set to False or use taxi_bridge_v6.py for standard 24kHz mode
# ═══════════════════════════════════════════════════════════════════════════
RASA_AUDIO_PROCESSING = os.environ.get("RASA_AUDIO_PROCESSING", "true").lower() in ("true", "1", "yes")
...
class TaxiBridgeRasa:
    def __init__(self, reader, writer):
        self.reader = reader
        self.writer = writer
        self.ws = None
        self.running = True
        self.audio_queue = deque(maxlen=200)  # Bounded queue
        self.call_id = f"rasa-{int(time.time() * 1000)}"  # Prefix with 'rasa' for easy log filtering
        self.phone = "Unknown"
        self.ast_codec = "ulaw"
    ...
    async def run(self):
        ...
        try:
            ...
            # Send init with Rasa mode enabled
            init_msg = {
                "type": "init",
                "call_id": self.call_id,
                ...
                "rasa_audio_processing": RASA_AUDIO_PROCESSING,  # True by default in this script
            }
            await self.ws.send(json.dumps(init_msg))
            self.init_sent = True
...
async def main():
    server = await asyncio.start_server(
        lambda r, w: TaxiBridgeRasa(r, w).run(),
        AUDIOSOCKET_HOST, AUDIOSOCKET_PORT
    )
    audio_mode = "Rasa (8→16kHz)" if RASA_AUDIO_PROCESSING else "Standard (8→24kHz)"
    logger.info(f"🚀 Taxi Bridge RASA MODE - 16kHz STT")
