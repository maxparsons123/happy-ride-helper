/**
 * taxi-realtime-bridge: Smooth Desktop Mode for Asterisk/Python Bridge
 * 
 * Optimized for ulaw (8kHz) audio from Python bridge with proper codec conversion.
 * Uses the smoother desktop timing constants for better interaction flow.
 */
...
// ---------------------------------------------------------------------------
// Audio helpers - BRIDGE OPTIMIZED for ulaw (8kHz)
// ---------------------------------------------------------------------------
...
# PASSENGER NUMBER HANDLING (CRITICAL)
When asked about passengers, the user will say a NUMBER. Common responses:
- "one" / "1" / "just me" → 1 passenger
- "two" / "2" / "me and my wife" → 2 passengers  
- "three" / "3" / "Boston Juice" / "ozempic juice" / "free" → 3 passengers
- "four" / "4" → 4 passengers
If unclear, assume 1 passenger.
...
// DESKTOP-SMOOTH TIMING CONSTANTS
const ECHO_GUARD_MS = 150;
const GREETING_PROTECTION_MS = 2000;
const SUMMARY_PROTECTION_MS = 6000;
const ASSISTANT_LEADIN_IGNORE_MS = 500;

// RMS thresholds
const RMS_NOISE_FLOOR = 650;
const RMS_BARGE_IN_MIN = 800;
const RMS_BARGE_IN_MAX = 25000;
...
async function handleConnection(socket: WebSocket, callId: string, callerPhone: string, language: string) {
  console.log(`[${callId}] 🌉 BRIDGE MODE: New connection from ${callerPhone} (language: ${language})`);
  ...
  const sendGreeting = () => {
    ...
    sessionState.greetingProtectionUntil = Date.now() + GREETING_PROTECTION_MS * 2;
  ...
  socket.onmessage = async (event) => {
    try {
      if (typeof event.data === "string") {
        ...
        // PRIMARY PATH: ulaw audio from Python bridge
        if (data.type === "audio" && data.audio) {
          sessionState.audioPacketsReceived++;
          audioDiag.packetsReceived++;
          
          // Decode base64 ulaw to PCM
          const binaryStr = atob(data.audio);
          const bytes = new Uint8Array(binaryStr.length);
          for (let i = 0; i < binaryStr.length; i++) bytes[i] = binaryStr.charCodeAt(i);
          
          // Convert ulaw (8kHz) to PCM16
          ...
          // Apply pre-emphasis for better STT, then resample 8k → 24k
        ...
        } else if (data.type === "input_audio_buffer.append" && data.audio) {
          // SECONDARY PATH: PCM16 @ 24kHz from C# bridge
          sessionState.audioPacketsReceived++;
          audioDiag.packetsReceived++;
          ...
          if (sessionState.audioPacketsReceived <= 5) {
            console.log(`[${callId}] 🎙️ PCM24k #${sessionState.audioPacketsReceived}: RMS=${rms.toFixed(0)}`);
          }
...
Deno.serve(async (req) => {
  ...
  if (url.pathname === "/health" || url.pathname.endsWith("/health")) {
    return new Response(JSON.stringify({ 
      status: "healthy",
      mode: "bridge",
      timing: { echo_guard: ECHO_GUARD_MS, greeting: GREETING_PROTECTION_MS, summary: SUMMARY_PROTECTION_MS },
  ...
  if (upgrade.toLowerCase() === "websocket") {
    const callId = url.searchParams.get("call_id") || `bridge-${Date.now()}`;
    ...
    console.log(`[${callId}] 🌉 Bridge WebSocket upgrade: caller=${callerPhone}`);
