...
public sealed class ALawRtpPlayout : IDisposable
{
    ...
    public event Action<string>? OnFault; // kept for interface compatibility
    ...
    // Stubs kept so CallSession.cs compiles without changes
    ...
    /// <summary>Flush any partial frame in the accumulator (padded with silence).</summary>
    public void Flush()
    {
        lock (_accLock)
        {
    ...
    public void Start()
    {
        ...
        SafeLog($"[RTP] v8.4 started (pure A-law, {JITTER_BUFFER_FRAMES * FRAME_MS}ms fixed buffer, nano-timing)");
    }
    ...
    /// Tier 3 (&lt;0.5ms):   Thread.SpinWait(20) — busy-wait for final sub-ms precision
    ///
