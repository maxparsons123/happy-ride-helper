...
public static class SpoofDetector
{
        ...
        double reportedSpeedKmh = -1)
    {
        ...
        if (prev != null)
        {
            ...
            var computedKmh = (km / dt) * 3600.0;

            // Unrealistic taxi speed
            if (computedKmh > 140) { risk += 0.55; flags.Add($"speed_{computedKmh:F0}kmh"); }
            else if (computedKmh > 110) { risk += 0.35; flags.Add($"speed_{computedKmh:F0}kmh"); }

            // Cross-check: big mismatch between reported speed and computed speed
            if (reportedSpeedKmh >= 0 && computedKmh > 5)
            {
                var ratio = reportedSpeedKmh / Math.Max(1, computedKmh);
                if (ratio < 0.3 || ratio > 3.0)
                {
                    risk += 0.20;
                    flags.Add($"speed_mismatch_reported={reportedSpeedKmh:F0}_computed={computedKmh:F0}");
                }
            }
