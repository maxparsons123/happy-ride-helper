...
public sealed class MicGateController
{
    ...
    /// Lower than RMS-based threshold because we use linear deviation, not squared.
    /// </summary>
    ...
    /// Slightly longer than naive 150ms to account for PSTN echo tails.
    /// </summary>
    ...
    public bool ShouldSendToOpenAi(byte[] frame, out bool isBargeIn)
    {
        ...
        // Prevent echo-trigger in first ~180ms
    ...
    /// <summary>
    /// Average absolute deviation for encoded G.711 bytes, centered on codec-specific silence byte.
    /// More stable than RMS in the companded (logarithmic) domain — avoids the squaring
    /// that amplifies noise in non-linear codecs like A-law/µ-law.
    /// </summary>
