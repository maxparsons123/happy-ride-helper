...
public sealed class CallSession : ICallSession
{
    ...
    /// <summary>Simple Levenshtein distance for short street name words.</summary>
    ...
    /// Returns null if no number found. Used as a hard geocoding filter sent to Gemini.
    /// </summary>
    private static string? GetSpokenHouseNumber(string? address)
    {
        if (string.IsNullOrWhiteSpace(address)) return null;
        var c = AddressParser.ParseAddress(address);
