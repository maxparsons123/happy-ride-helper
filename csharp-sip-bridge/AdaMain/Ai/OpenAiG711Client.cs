...
/// No resampling - raw A-law frames in and out.
/// Version 1.0 - Native G.711 mode
/// </summary>
public sealed class OpenAiG711Client : IOpenAiClient, IAsyncDisposable
{
    public const string VERSION = "1.0-g711";
    ...
    public async Task ConnectAsync(string callerId, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("Connecting to OpenAI G.711 v{Version} (caller: {CallerId}, lang: {Lang})", 
    ...
    /// <summary>Send raw G.711 A-law RTP audio directly to OpenAI.</summary>
            ...
            switch (type)
            {
                ...
                case "response.audio.delta":
                    if (doc.RootElement.TryGetProperty("delta", out var delta))
                    {
                        ...
                        if (b64.Length > 0)
                        {
                            // Native G.711 A-law - direct passthrough!
                            ...
                            for (int i = 0; i < alawBytes.Length; i += 160)
                            {
                                ...
                                // Pad short frames with A-law silence
                                if (frameSize < 160)
    ...
    private async Task HandleToolCallAsync(JsonElement root)
    {
        ...
        // Brief delay for SIP jitter buffers before next response
        await Task.Delay(40);
    ...
    {
        // Native G.711 A-law - 8kHz passthrough mode
        var config = new
        {
            type = "session.update",
            session = new
            {
                ...
                input_audio_format = "g711_alaw",   // Native A-law input
                output_audio_format = "g711_alaw",  // Native A-law output
                input_audio_transcription = new { model = "whisper-1" },
                turn_detection = new
                {
                    type = "server_vad",
                    threshold = 0.4,
                    prefix_padding_ms = 300,      // Faster VAD for telephony
    ...
    private static string GetDefaultSystemPrompt() => """
        You are Ada, a friendly taxi booking assistant for Voice Taxibot. Version 2.2 G.711.
