...
export function DispatchRadio({ publish, mqttConnected }: DispatchRadioProps) {
  ...
  const startPtt = useCallback(async () => {
    if (pttActive || !mqttConnected) return;
    setPttActive(true);
  ...
  }, [pttActive, mqttConnected, publish, addLog]);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[900] w-56 bg-[#0a0a0a]/95 border border-[#333] rounded-2xl p-4 backdrop-blur-xl shadow-2xl">
      {/* Header */}
      ...
      <div className={`text-center text-xs mb-3 font-semibold ${
        ...
        {pttActive ? '🔴 BROADCASTING TO ALL DRIVERS...' : mqttConnected ? 'Ready — Hold to broadcast' : 'MQTT disconnected'}
      </div>
